...
function NavbarComponent({ showPurchaseModal }) {
  ...
  const MobileNavLink = ({ to, icon, children }) => (
    ...
    >
      {React.createElement(icon)} {children}
    </NavLink>
  );
      ...
      <nav className="navbar">
        <div className="navbar-left">
          ...
          <div className={`nav-links ${mobileMenuOpen ? 'mobile-menu-open' : ''}`}>
            <MobileNavLink to="/quotations" icon={FaFileInvoice}>
              Quotations
            </MobileNavLink>
            <MobileNavLink to="/tickets" icon={FaTicketAlt}>
              Tickets
            </MobileNavLink>
            <MobileNavLink to="/logtime" icon={FaClock}>
              Log Time
            </MobileNavLink>
            <MobileNavLink to="/challan" icon={FaClipboardList}>
              Challan
            </MobileNavLink>
            ...
            {user && user.role !== "user" && (
              ...
              >
                <MobileNavLink to="/itemslist" icon={FaBoxOpen}>
                  Items List
                </MobileNavLink>
                {(showItemsDropdown || mobileMenuOpen) && (
                  <div className="dropdown-menu">
                    <div onClick={handleViewAllItems}>
                      View All Items
                    </div>
                    <div onClick={() => { showPurchaseModal(); closeMobileMenu(); }}>
                      Update Stock
                    </div>
                    <div onClick={handlePurchaseHistoryClick}>
                      Purchase History
                    </div>
                  </div>
                )}
            ...
            {user && user.role !== "user" && (
              <MobileNavLink to="/users" icon={FaUsers}>
                Users
              </MobileNavLink>
            )}
