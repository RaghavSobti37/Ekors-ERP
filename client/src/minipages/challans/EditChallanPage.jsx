...
const EditChallanPage = () => {
  ...
  const fetchChallanDetails = useCallback(async () => {
    ...
    try {
      ...
      setFormData({
        ...
        media: null,
        existingDocument: data.document ? { name: data.document.originalName, id: data._id } : null,
      });
      setError(null);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      frontendLogger.info("challanActivity", `Challan ${challanId} updated`, authUser, { action: "UPDATE_CHALLAN_SUCCESS" });
      navigate("/challans");
    } catch (err) {
      const errorMessage = handleApiError(err, "Failed to update challan.", authUser, "challanActivity");
      setError(errorMessage);
      toast.error(errorMessage);
      frontendLogger.error("challanActivity", `Failed to update challan ${challanId}`, authUser, { action: "UPDATE_CHALLAN_FAILURE", error: err });
    } finally {
      setIsLoading(false);
    }
  };

  if (isLoading && !formData.companyName) { // Show full page loader only on initial load
    ...
    <ReusablePageStructure
      ...
      footerContent={
        <>
          ...
          <BsButton type="submit" form="challan-form" variant="primary" disabled={isLoading}>
            {isLoading ? <><Spinner as="span" animation="border" size="sm" role="status" aria-hidden="true" /> Updating...</> : "Update Challan"}
          </BsButton>
        </>
      }
    >
      ...
      <Form id="challan-form" noValidate validated={formValidated} onSubmit={handleSubmit}>
        <Card className="mb-3">
          ...
          <Card.Body>
            <Form.Group className="mb-3">
              <Form.Label>Search and Select Client (Optional: to update client details)</Form.Label>
        ...
        <Card className="mb-3">
          ...
          <Card.Body>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Upload New Document (Optional: replaces existing)</Form.Label>
              <Form.Control type="file" name="media" accept="image/*,application/pdf" onChange={handleFileChange} />
              {formData.media && <Form.Text className="text-muted">Selected file to upload: {formData.media.name}</Form.Text>}
              {!formData.media && formData.existingDocument && (
                <Form.Text className="text-success d-block mt-1">Current document: {formData.existingDocument.name}</Form.Text>
              )}
            </Form.Group>
          </Card.Body>
        </Card>
