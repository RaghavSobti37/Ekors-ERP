...
export default function Quotations() {
  ...
  const handleCreateTicket = useCallback(
    async (quotation) => {
      ...
      if (!quotation.shippingAddress?.address1) {
        toast.info(
          "Billing address will be used as shipping address since no shipping address was defined."
        );
      }
      ...
      try {
        ...
        const response = await apiClient(
          `/tickets/from-quotation/${quotation._id}`,
          {
            method: "POST",
            body: {
              userId: user?.id,
            },
            // No need to explicitly set status here, it should be handled by the backend logic
          }
        );
        const statusChange = await apiClient(`/quotations/${quotation._id}`, {
          method: "PATCH",
          body: {
            status: "running",
          },
        ...
        // After successful ticket creation and navigation, re-fetch quotations to update status
        fetchQuotations(currentPage, itemsPerPage);
        setIsLoading(false);
      } catch (error) {
        toast.error(
  ...
  const handleEdit = useCallback(
    // This is for editing an existing quotation
    async (quotation) => {
      ...
      const quotationDataForForm = {
        ...
        goods: quotation.goods.map((item) => ({
          ...
          originalPrice: Number(
            item.sellingPrice || item.originalPrice || item.price
          ),
          ...
          originalItem:
            item.originalItem?._id || item.originalItem || item.itemId,
  ...
  return (
    <div>
      ...
      {/* Show spinner during data fetch or auth loading */}
          ...
          <ReusableTable
            columns={[
                ...
                : []),
              {
                ...
                sortable: true,
                renderCell: (item) =>
                  formatDateForInputHelper(item.validityDate),
              },
              {
                key: "status",
                header: "Status",
            ...
            renderActions={(quotation) => (
              <ActionButtons
                ...
                onView={() =>
                  navigate(`/quotations/preview/${quotation._id}`, {
