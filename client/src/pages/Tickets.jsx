...
export const PIActions = ({ ticket, onPreviewPI }) => {
  const handleDownloadWord = useCallback(async () => {
    ...
    if (
      finalAmountForDoc === undefined ||
      finalAmountForDoc === null ||
      roundOffForDoc === undefined
    ) {
      const decimalPartForDoc =
        (ticket.grandTotal || 0) - Math.floor(ticket.grandTotal || 0);
      roundOffForDoc = 0;
      finalAmountForDoc = ticket.grandTotal || 0;
      if (decimalPartForDoc !== 0) {
        if (decimalPartForDoc < 0.5) {
    ...
    const ticketForDoc = {
      ...ticket,
      finalRoundedAmount: finalAmountForDoc,
      roundOff: roundOffForDoc,
    };
    try {
      console.log("Generating PI Word document...", ticketForDoc);
      const doc = generatePIDocx(ticketForDoc);
      console.log("PI Word document generated successfully.");
      const blob = await docx.Packer.toBlob(doc);
      saveAs(
        blob,
        `PI_${ticketForDoc.ticketNumber || ticketForDoc.quotationNumber}.docx`
      );
    } catch (error) {
  ...
  if (
    finalAmountForPdfPreview === undefined ||
    finalAmountForPdfPreview === null ||
    roundOffForPdfPreview === undefined
  ) {
    const decimalPartForPdf =
      (ticket.grandTotal || 0) - Math.floor(ticket.grandTotal || 0);
    roundOffForPdfPreview = 0;
    finalAmountForPdfPreview = ticket.grandTotal || 0;
    if (decimalPartForPdf !== 0) {
      if (decimalPartForPdf < 0.5) {
  ...
  const ticketForPdfPreview = {
    ...ticket,
    finalRoundedAmount: finalAmountForPdfPreview,
    roundOff: roundOffForPdfPreview,
  };
  ...
  return (
    <ActionButtons
      ...
      customActions={[
        {
          label: "Preview PI",
          handler: () => onPreviewPI(ticketForPdfPreview),
          icon: FaFilePdf,
...
export default function Dashboard() {
  ...
  const handleDelete = useCallback(
    async (ticketToDelete) => {
      if (!authUser || authUser.role !== "super-admin") {
        const msg = "Permission denied to delete ticket.";
        setError(msg);
        toast.warn(msg);
        return;
      }
      if (window.confirm(`Delete ticket ${ticketToDelete.ticketNumber}?`)) {
        ...
        try {
          ...
          setError(null);
          const successMsg = `Ticket ${ticketToDelete.ticketNumber} deleted.`;
  ...
  const handleEdit = useCallback(
    (selectedTicketToEdit) => {
      navigate(`/tickets/edit/${selectedTicketToEdit._id}`, {
        state: { ticketDataForForm: selectedTicketToEdit },
      });
    },
    [navigate]
  );
  ...
  const handleTransfer = useCallback(
    (ticketToTransfer) => {
      navigate(`/tickets/transfer/${ticketToTransfer._id}`, {
        state: { ticketDataForTransfer: ticketToTransfer },
      });
    },
    [navigate]
  );
  ...
  const reportButtonElement = (authUser?.role === "admin" ||
  ...
  return (
    <div>
      <Navbar />
      <LoadingSpinner
        show={isLoading || authLoading || isCompanyInfoLoading}
      />{" "}
            ...
            renderActions={(ticket) => {
              ...
              return (
                <ActionButtons
                  ...
                  onDelete={
                    authUser?.role === "super-admin" ? handleDelete : undefined
                  }
                  customPIActions={
                    <PIActions
                      ticket={ticket}
                      onPreviewPI={handlePreviewPIFromList}
                    />
                  }
                  onTransfer={
                    canTransferThisTicket ? handleTransfer : undefined
                  }
                  isLoading={isLoading || authLoading}
                  disabled={{
                    edit: !canModifyTicket || isLoading || authLoading,
                    transfer:
                      !canTransferThisTicket || isLoading || authLoading,
                  }}
                  size="sm"
                />
              );
      ...
      {showPIPreviewModal && ticketForPIPreview && companyInfo && (
        ...
        >
          ...
          >
            <Modal.Title>
              PI Preview -{" "}
              {ticketForPIPreview.ticketNumber ||
                ticketForPIPreview.quotationNumber}
            </Modal.Title>
          </Modal.Header>
          ...
          <Modal.Footer>
            <Button
              variant="secondary"
              onClick={() => setShowPIPreviewModal(false)}
            >
              Close
            </Button>
          </Modal.Footer>
